...
`);
    ...
    let emoji = await ConfigMessage.awaitReactions(
      ...
      client.sendTime(
        message.channel,
        "❌ | **Kuch jyada time le liya tune respond karne m. agar settings edit karni h to , command fir se run kar :) **"
      );
    ...
    if (em._emoji.name === "1️⃣") {
      await client.sendTime(
        message.channel,
        "Prefix ko kise replace karu ye bata :) "
      );
      ...
      if (!prefix.first())
        return client.sendTime(
          message.channel,
          "Bhot der laga di javab dene m tune :)."
        );
      prefix = prefix.first();
